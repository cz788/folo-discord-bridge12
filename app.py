...
FILTER_KEYWORDS = [k.strip() for k in os.getenv("FILTER_KEYWORDS", "AI,人工智能").split(",")]
EXCLUDE_KEYWORDS = [k.strip() for k in os.getenv("EXCLUDE_KEYWORDS", "广告").split(",")]

async def send_to_discord(entry, feed):
...
def should_forward(entry, feed):
    ...
    for k in EXCLUDE_KEYWORDS:
        if k.lower() in text: return False, f"排除: {k}"
    if FILTER_KEYWORDS and FILTER_KEYWORDS[0]:
        for k in FILTER_KEYWORDS:
            if k.lower() in text: return True, f"匹配: {k}"
        return False, "无关键词"
    return True, "全部"
...
async def receive():
    ...
    if ok: await send_to_discord(entry, feed)
